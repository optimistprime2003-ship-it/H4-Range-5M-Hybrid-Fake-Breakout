...
# The first 4H candle runs 09:00–13:00 NY. The range isn't
    ...
    for symbol in RANGE_PAIRS:
        ...
        try:
            ...
            if session_data.empty:
                ...
                # Diagnostic: show the ACTUAL 4H candle timestamps
                # Twelve Data returned, so we can see what time
                # forex candles really land on instead of guessing.
                actual_times = df_4h["datetime"].tolist()

                logging.warning(
                    f"{symbol} — actual 4H candle timestamps received: "
                    f"{actual_times}"
                )
